...
internal sealed class NativeCharacterRenderEngine : IDisposable
{
    ...
    public NativeCharacterRenderEngine(Dispatcher dispatcher)
    {
    ...
    internal NativeCharacterScene Scene => _scene;

    public CharacterRenderSnapshot GetSnapshot(string characterId) =>
        _scene.TryGet(characterId, out NativeCharacterState? state)
    ...
    private static CharacterRenderSnapshot CreateSnapshot(
        NativeCharacterState state) =>
        new(
            state.Config.Id,
            state.IsLoading,
            state.IsVisible,
            state.CachedAnimationNames.ToArray(),
            state.MaxScale,
            state.CurrentScale);

    private void NotifyCharacterState(NativeCharacterState state) =>
        CharacterStateChanged?.Invoke(CreateSnapshot(state));
