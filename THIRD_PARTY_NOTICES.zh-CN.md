...
1. 保留发行包根目录下的 `LICENSE`、`NOTICE`、`THIRD_PARTY_NOTICES.md`（英）/ `.zh-CN.md`（中）、
   `ASSETS.md` / `ASSETS.zh-CN.md` 与整个 `licenses/` 目录，不要删除或改写其中的第三方许可证原文。
2. 不要把第三方组件（尤其 Spine 运行时）单独抽出当成独立产品分发，也不要给它换许可证。
3. 不要往包里添加任何游戏角色素材；素材不属于本项目，本项目无权为你授权（见 `ASSETS.zh-CN.md`）。
