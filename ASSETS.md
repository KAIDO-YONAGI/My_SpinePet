...
- The bundled import/unpack tooling — UnityFS bundle import, icon download, and
  — only processes files **you already hold**. It does not find, download or
  unlock any content for you.

## 4. Credits

- **nikke-db** (https://github.com/Nikke-db/Nikke-db.github.io) — a community
  project that has stayed free for years, with no ads and no donation links. The
  character assets this project uses come from material it publishes publicly
  (Live2D, sprites and their indexes) rather than from any unpacking done here;
  those assets are not distributed with this project, are not owned by nikke-db
  either, and this credit implies no licence or endorsement from it. The helper
  script and `resources/nikkedb/` mirror used at the time are local-only tools and
