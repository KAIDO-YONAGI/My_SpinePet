...
5. **下拉框验收**：不能只断言 ViewModel 字符串。必须验证收起标题、
   ...
   Reset All、连续第二次 Reset，以及至少两轮
   `Normal -> Battle -> Normal` 往返。禁止以硬编码选项或
   `空 -> 当前值` 的源状态抖动作为绑定修复；选项与选中项必须由单一视图
   写入者作为一次控件提交应用。纯视觉调整（例如隐藏下拉滚动条）也必须
   确认不改变 `ItemsSource`、`SelectedItem` 以及滚轮/键盘滚动行为。
