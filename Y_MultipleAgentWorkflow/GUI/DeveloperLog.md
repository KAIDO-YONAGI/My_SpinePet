...
## 2026-08-24：左侧预览栏增加平滑滚动动画

- 用户澄清需求为保留现有滚动与选中逻辑，只改善左侧预览栏滑动时的视觉效果。
- 修复：`CharacterPreviewNavigationController` 的居中滚动仍使用原有目标偏移、
  两列槽位、滚轮步进和边界钳制，只把瞬时 `ScrollToVerticalOffset` 改为
  180ms `CubicEase EaseOut` 动画。连续滚轮会替换当前动画目标，不改变选中项
  计算；动画期间暂时抑制滚动跟随回写，避免动画过程触发额外选择。
- 定位角色的 Reveal 流程等待动画完成后再继续可见性确认，避免重复重启动画。
- 验证：预览导航目标测试 `28/28` 通过；Debug 全量测试 `317/325` 通过，
  另有 8 项在无桌面字体 URI 的环境中触发既有
  `MS.Internal.FontCache.Util` 初始化失败；Release Build 0 警告、0 错误；
  Publish 成功；`Run.bat` 启动实例 PID `32828` 正常运行。
- 本次实际影响 GUI 预览滚动呈现，维护计数：`1/5 -> 2/5`。

## 2026-08-24：预览列表自动选择后的旧框选残留修复

- 用户复报：先点击左侧预览卡片，再用滚轮触发自动选择移开时，旧卡片的
  框选仍会保留，界面同时出现两个带框条目。
- 根因：卡片模板分别用 `IsSelected` 绘制选中态、用
  `IsKeyboardFocused` 绘制焦点框。鼠标点击会把键盘焦点留在旧卡片上，
  滚动导航随后只更新 `SelectedItem`，因此旧卡片虽已取消选中，仍继续显示
  独立焦点框。
- 修复：`CharacterCardItemStyle` 的焦点框改为仅在
  `IsKeyboardFocused=True` 且 `IsSelected=True` 时显示。滚轮、滚动条及
  键盘导航仍由原选择控制器更新唯一选中项；旧卡片失去选中后立即移除框线。
- 回归钉住：`CharacterLibraryLayoutTests` 新增 XAML 级断言，要求显示
  `FocusChrome` 的触发器同时包含键盘焦点与选中条件，防止再次退回两套
  相互独立的框选视觉状态。
- 验证：角色库布局与预览导航目标测试 `41/41` 通过；Release Build
  0 警告、0 错误；Publish 成功，发布目录为
  `SpinePet-Release-2026-08-24-23 18 29`；`Run.bat` 返回 0，运行实例
  PID `33004` 正常启动。
- 本次实际影响 GUI 角色卡片选择视觉，维护计数（新周期）：
  `0/5 -> 1/5`。
