...
### 6.1 Codex 原生 Agent 协作适配

Codex 可以提供任务之间的原生消息通道。它是租约协议的可选适配层，不是
新的权威来源，也不是自动文件锁：

1. Agent 先读取租约状态。发现活动或等待中的重叠租约时，在有可用 Codex
   任务通道的情况下，向持有任务发送简短冲突通知。
2. 通知至少说明重叠路径或共享资源、自己的访问模式、当前任务摘要，以及
   请求释放、协调范围还是等待回复。
3. `read + read` 或路径不相交时不必发送消息。消息不能授权绕过租约冲突。
4. 对方确认完成后，等待其 `Release`，再重新执行 `Status`；不得只凭消息
   推断范围已经释放。
5. `suspectedStale` 只代表疑似失活，不代表可以自动接管。确认旧任务已经
   停止后仍须遵守脚本的显式 stale removal 规则，或把冲突交给用户决定。
6. 解除阻塞后发送简短事实更新；完成时先 `Release`，再通知等待任务。消息
   只传递已核验事实、改动范围和验证结果，推断必须标明为假设。
7. 没有原生通信能力时，退回租约检查和用户报告，不假设客户端之间存在后台
   监听或自动协调。

该适配使共享工作树中的 Codex 任务可以把租约状态连接到任务沟通，但仍不能
消除不遵守协议时的覆盖、竞态和最终合并冲突。
