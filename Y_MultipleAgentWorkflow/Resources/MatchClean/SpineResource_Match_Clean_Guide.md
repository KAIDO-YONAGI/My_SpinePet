...
3. 点击角色身体触发动画，点击周围透明区域不触发
   （两阶段命中：先包围矩形、再渲染三角形）；
4. 点击动画结束后从头重播当前有效的常驻动画；没有有效常驻动画时从头重播
   默认待机动画（见 §6）；
...
Animation 下拉框中的用户选择是角色的常驻动画。点击动画只做临时覆盖：

1. 进入临时覆盖时确定恢复目标：优先使用 Animation 下拉框保存的有效动画，
   否则优先选择名称精确为 `idle` 的动画，再选择 `idle*` 变体，
   最后回退到资源中的第一个动画；
2. 点击动画以非循环方式播放；
3. 点击动画结束后，把恢复目标以循环方式排到同一轨道，并从第 0 帧重新开始；
4. 连续点击期间沿用第一次临时覆盖确定的恢复目标，避免点击链改变待机衔接；
5. 用户在 Animation 下拉框改选动画时，清除旧临时队列，立即采用新选择作为
   常驻动画；后续点击结束仍从该动画第 0 帧开始。

这里的“恢复”只表示恢复到常驻动画名称和循环策略，不表示恢复点击前的播放进度。
`idle` 选择规则既用于没有有效常驻动画时的初始化，也用于临时点击动画结束后的
默认回退。配置动画本身不会被点击等临时动画覆盖或写回配置。
...
- 点击动画非循环播放，但没有从头排队当前常驻动画或默认待机动画；
- 临时播放失败后没有清理轨道状态，导致后续点击无法重试。
