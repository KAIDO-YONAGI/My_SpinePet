...
1. 首次覆盖前保存当前动画名、循环状态和播放进度；
2. 点击动画以非循环方式播放；
3. 点击结束后恢复保存的状态，不强制切回 `idle`；
4. 连续点击沿用首次覆盖前的恢复目标；
5. 用户在 Animation 下拉框改选动画时，清除旧临时队列并立即采用新选择。

`idle` 选择规则只用于角色没有有效常驻动画时的初始化和重置，不作为每次
临时动画结束后的固定回退。
...
- 点击动画非循环播放，但没有保存并排队恢复播放前的常驻动画状态。
